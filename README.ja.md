...
<p align="center">
  <em>Markdown を、静かに。</em>
</p>

<p align="center">
  <sub>Android のための、書くことに集中できるエディター — デスクトップ版 <a href="https://github.com/marktext/marktext">MarkText</a> の体験を、スマートフォンに合わせて仕立てました。</sub>
</p>
...
MarkText for Android は、MarkText のライブプレビュー型 Markdown 編集を、ネイティブ
アプリとして書き直すことなくスマートフォンに持ち込みます。エディターの中核は
MarkText のオープンソースコアである Muya で、モバイル向けに大きく手を加えています。
大きなドキュメントでも高速に動作し、スマートフォンの画面幅に合わせてレイアウトを
組み直し、デスクトップには不要だったタッチ選択とツールバーを備えました。書いた内容は
...
- 本物のライブプレビュー（WYSIWYG）編集 — 単なるテキストボックスではなく、
  デスクトップ版 MarkText の執筆体験そのものです。
...
- **作業内容を決して失いません。** 自動保存と復元用下書きがすべての変更を保持し、
  オール・オア・ナッシングのアトミックな書き込みにより、保存が中断されても
  書きかけや破損したファイルが残ることはありません。
- **デフォルトでプライベート。** アカウント不要、クラウドなし、テレメトリーなし。
  すべては端末の中にとどまります。
- **軽量。** 重いネイティブスタックではなく、無駄のない Vue + Capacitor シェルを
  採用したことで、アプリ全体は約 7.8 MB — 小さく軽く、それでいてフル機能です。
...
カスタマイズは細部まで行き届いており、執筆中に触れるバーそのものにまで及びます:

- **ツールバーを自作できます。** 下部のクイックバーはコマンド一覧から組み立て、
  ドラッグで並べ替えられます。選択ツールバー — テキストを選択したときに現れる
  クリップボードバー — にも独自のコマンドを 1 行または 2 行で配置できます。
- **テーマと文字。** ライト、ダーク、カスタムテーマ（`ayu-light`、`one-dark`）。
  フォント、フォントサイズ、行の高さ、行幅、書字方向（LTR または RTL）を調整
  できます。
- **Markdown も好みのままに。** リストマーカーとインデント、見出しスタイル、
  フロントマター形式（YAML/TOML/JSON）、脚注、上付き/下付き文字、HTML
  レンダリング、GitLab 互換性。
...
- **ファイルは元の場所のまま。** システムピッカーを通じて任意のストレージ
  ...
  やり取りできます — 取り込みと書き出しを行き来する手間も、サンドボックス内の
  二重コピーもありません。
- **親指のための設計。** 片手で無理なく届き、エディターを主役にした落ち着いた
  レイアウト。幅の広い表はページを揺らさず、表自体が横にスクロールします。
- **アクセシブルで、控えめ。** WCAG 2.2 AA を満たす静かなグラファイトデザイン。
  フォーカス順序は目で追え、モーション低減の設定も尊重します。
...
> [!IMPORTANT]
> 初の署名付き公開リリース **v0.1.0** が利用可能です。その APK はリポジトリの
> リリースワークフローでビルドされ、公式のリリース証明書に固定されており、
> Android 14 でのクリーンインストールと同一鍵でのアップグレード検証に合格して
> います。
...
[Android Studio](https://developer.android.com/studio)（Android SDK — 最小 API 24、
ターゲット 36 — と JDK）が必要です。
...
Issue と Pull Request を歓迎します。一つひとつの変更は焦点を絞り、意味のある
箇所にはテストを追加してください。
...
<p align="center"><sub><em>Markdown を、静かに。</em></sub></p>
