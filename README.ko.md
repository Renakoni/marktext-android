...
<p align="center">
  <sub>Android를 위한 집중형 편집기 — 데스크톱 <a href="https://github.com/marktext/marktext">MarkText</a> 경험을 폰에 맞게 빚었습니다.</sub>
</p>
...
MarkText for Android는 MarkText의 실시간 미리 보기 Markdown 편집을 네이티브
앱으로 다시 만들지 않고 폰으로 가져옵니다. 편집기는 MarkText의 오픈 소스 코어인
Muya로, 모바일에 맞게 크게 손질했습니다. 큰 문서에서 더 빠르게 동작하고, 폰
화면 너비에 맞춰 다시 배치했으며, 데스크톱에는 필요 없던 터치 선택과 도구
모음을 더했습니다. 작성한 내용은 데스크톱과 같은 품질로 렌더링되며, 이 모든
것이 한 손을 위해 설계된 인터페이스 안에서 이루어집니다.
...
- 진짜 실시간 미리 보기(WYSIWYG) 편집 — 단순한 텍스트 상자가 아닌, 데스크톱
  MarkText의 글쓰기 경험 그대로.
...
- **작업을 절대 잃지 않습니다.** 자동 저장과 복구 초안이 모든 변경 사항을
  지키고, 전부 아니면 전무의 원자적 쓰기 덕분에 저장이 중단되어도 반쯤 쓰였거나
  손상된 파일이 남지 않습니다.
- **기본이 프라이버시.** 계정도, 클라우드도, 원격 측정도 없습니다. 모든 것이
  기기 안에 머뭅니다.
- **가볍습니다.** 무거운 네이티브 스택 대신 집중된 Vue + Capacitor 셸을 사용해
  앱 전체가 약 7.8 MB — 작고 가볍지만 기능은 온전합니다.
...
여기서는 사용자 정의가 깊숙이 이어집니다. 글을 쓰는 동안 손이 닿는 막대까지
직접 바꿀 수 있습니다:

- **나만의 도구 모음 만들기.** 명령 풀에서 하단 빠른 도구 막대를 구성하고
  드래그로 순서를 바꿉니다. 강조된 텍스트 위에 나타나는 클립보드 막대인 선택
  도구 모음에도 원하는 명령을 한 행 또는 두 행으로 담을 수 있습니다.
- **테마와 글꼴.** 라이트, 다크, 사용자 정의 테마(`ayu-light`, `one-dark`);
  글꼴 종류, 크기, 줄 높이, 줄 너비, 텍스트 방향(LTR 또는 RTL)을 조정할 수
  있습니다.
- **취향대로 쓰는 Markdown.** 목록 기호와 들여쓰기, 제목 스타일, front matter
  형식(YAML/TOML/JSON), 각주, 위/아래 첨자, HTML 렌더링, GitLab 호환성.
...
- **파일은 제자리에 그대로.** 시스템 선택기를 통해 어떤 저장소 제공자에서든
  ...
  가져오기/내보내기의 번거로움도, 샌드박스 속 두 번째 사본도 없습니다.
- **엄지를 위한 설계.** 한 손으로 편안하게 닿는 배치와 차분한 편집기 중심
  레이아웃. 넓은 표는 페이지를 밀어내는 대신 자체적으로 스크롤됩니다.
- **접근성 있게, 절제되게.** WCAG 2.2 AA를 충족하는 조용한 그래파이트 디자인,
  뚜렷한 포커스 순서, 모션 줄이기 설정 존중.
...
[Android Studio](https://developer.android.com/studio)(Android SDK — 최소 API
24, 대상 36 — 및 JDK)가 필요합니다.
...
이슈와 풀 리퀘스트를 환영합니다. 각 변경은 한 가지에 집중하고, 의미가 있는
곳에는 테스트를 더해 주세요.
...
<p align="center"><sub><em>조용히, Markdown.</em></sub></p>
